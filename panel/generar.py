...
def datos_reservados() -> dict:
    ...
    if not url or not key:
        print("  aviso: sin credenciales de Supabase; el tablero reservado saldrá sin cifras.")
        return {}
    try:
        ...
        print(f"  bloques reservados leídos: {', '.join(d.keys()) or 'ninguno'}")
        return d
    except Exception as e:
        print(f"  aviso: no se pudieron leer los datos reservados ({e}).")
        return {}
...
def main() -> int:
    ...
    for slug, (plantilla, nombre, objetivo, icono, orden, reservado) in TABLEROS.items():
        ...
        if chr(0) in html:
            print(f"  aviso: se retiraron caracteres nulos de {plantilla}")
